...
// ✅ Define glucose ranges as constants
...
export class ClinicOutcomesService {
  ...
  getOutcomes(range: OutcomeKey) {
    const data: Record<OutcomeKey, {
      patients: number;
      tir: { veryLow: number; low: number; target: number; high: number; veryHigh: number }; // ✅ Added veryHigh
      gmi: { good: number; moderate: number; bad: number };
      lastUpdated: string;
    }> = {
      30: {
        patients: 120,
        tir: { veryLow: 2, low: 0, target: 81, high: 15, veryHigh: 2 }, // ✅ No overlap: only veryLow shown
        gmi: { good: 72, moderate: 23, bad: 5 },
        lastUpdated: "2025-11-06T15:00:00Z"
      },
      60: {
        patients: 150,
        tir: { veryLow: 0, low: 5, target: 75, high: 17, veryHigh: 3 }, // ✅ No overlap: only low shown
        gmi: { good: 65, moderate: 27, bad: 8 },
        lastUpdated: "2025-09-23T09:15:00Z"
      },
      90: {
        patients: 180,
        tir: { veryLow: 0, low: 0, target: 70, high: 26, veryHigh: 4 }, // ✅ No overlap: both high and veryHigh visible
